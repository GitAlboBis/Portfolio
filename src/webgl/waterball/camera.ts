...
export class Camera {
    ...
    // opts let the embed disable drag-orbit / wheel-zoom (which fight the site scroll)
    // and track hover on `window` so the poke works even though the canvas is a
    // pointer-events:none background. Defaults match the original standalone demo.
    constructor (canvas: HTMLCanvasElement, opts: { orbit?: boolean; zoom?: boolean; hoverTarget?: EventTarget } = {}) {
        this.canvas = canvas;
        this.ac = new AbortController();
        const signal = this.ac.signal;
        const { orbit = true, zoom = true, hoverTarget = canvas } = opts;
        ...
        // hover ALWAYS tracked (drives the fluid poke). On `window` for the embed so a
        // pointer-events:none background canvas still gets cursor position.
        hoverTarget.addEventListener("mousemove", ((event: MouseEvent) => {
